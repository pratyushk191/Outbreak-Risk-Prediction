...
```
.
├── disease spread  feasibility model.py   # Training script — RF + GBM ensemble, oversampling, temporal split
├── predict_outbreak_risk.py               # Inference — generates predictions CSV
├── data_fusion.py                         # Multi-dataset fusion + Rt estimator + 4-week forecast + new features
├── demographics.py                        # Embedded country-level demographic data (115+ countries)
├── api_app.py                             # FastAPI REST server (8 endpoints)
...
│
└── outputs/
    ├── outbreak_risk_model.joblib         # Trained model
    ├── optimal_threshold.txt              # Tuned prediction threshold
    ├── metrics.txt                        # Accuracy, F1, classification report
    ├── feature_importances.csv            # Top 30 features (excl. country dummies)
    └── latest_outbreak_risk_predictions.csv
```
...
- **Target label:** `outbreak_risk = 1` if next week's cases grow ≥ 20% vs current week
...
- **Algorithm:** Soft-vote ensemble — `RandomForestClassifier` (200 trees, depth 12) + `GradientBoostingClassifier` (150 trees, depth 4, lr=0.10)
- **Preprocessing:** Median imputation for numerics, mode + one-hot encoding for location; `float64→float32` memory optimisation
- **Split:** Temporal 80/20 split (most recent 20% of dates = test) — prevents lag-feature leakage across time
- **Oversampling:** Minority class randomly oversampled to 25% of majority on the **train set only**
- **Threshold tuning:** Decision threshold maximises outbreak-class F1 subject to `recall ≥ 0.50` — prevents the model from flagging everything as an outbreak (v1 had precision=0.10 because the recall floor was missing)

**Results (v2 vs v3):**

| Metric | v1 | v2 | v3 (current) |
|---|---|---|---|
| Label noise | High | Fixed (100-case floor) | Same |
| Split method | Random | Temporal (no leakage) | Same |
...
| Booster | GBM (no imbalance weight) | GBM | **RF (balanced_subsample) + GBM (sample_weight=8x)** |
| Threshold strategy | Max F1 | Recall ≥ 0.50 floor | **PR curve: precision ≥ 0.20 AND recall ≥ 0.35** |
| Ensemble weights | 1:1 | 1:1 | **1:2 RF:GBM** |
| F1 (outbreak class) | 0.17 | 0.17 | **0.48** |
| Precision (outbreak) | 0.10 | 0.11 | **0.32** |
| Recall (outbreak) | 0.98 | 0.50 | **0.95** |
| PR-AUC | not reported | 0.11 | **0.38** |
| ROC-AUC | — | 0.97 | **0.96** |
| Primary metric | Accuracy (wrong) | Accuracy (wrong) | **PR-AUC** |
| Smoke test | No | No | **Yes (load → predict_proba → assert range)** |
| Rolling MA leakage | Yes | Yes | **Fixed** |
...
```
[Raw CSVs] → data_fusion.py (fuse + ffill vax + impute demographics)
                                      ↓
                    build_temporal_features() + compute_rt()
                                      ↓
               [Lag features + Rt + outbreak_risk label]
                                      ↓
         "disease spread feasibility model.py" (train + threshold tune)
                                      ↓
         outputs/outbreak_risk_model.joblib
         outputs/optimal_threshold.txt
         ...
         outputs/latest_outbreak_risk_predictions.csv
                                      ↓
              dashboard.py (6 tabs) / api_app.py (REST)
```
